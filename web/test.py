...
def test_create(request):
    ...
    if request.method == "POST":
        ...
        if test_obj.is_valid():
            ...
            test_obj.instance.concurrency = re.findall("Concurrency:(.*)Successful transactions:", out, re.S)[0].strip()  # 实际最高并发连接数
            test_obj.instance.successful = re.findall("Successful transactions:(.*)Failed transactions:", out, re.S)[0].strip()  # 成功次数
            test_obj.instance.failed = re.findall("Failed transactions:(.*)Longest transaction:", out, re.S)[0].strip()  # 失败次数
            test_obj.save()
